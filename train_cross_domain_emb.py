...
parser.add_argument('--clip_length', default=0, type=int, metavar='N',
                    help='clip length')
...
def main():
    ...
    # create model
    # doc_text_model = models.__dict__['Roberta'](
    #     model_type='rbt6',
    #     embedding_size=0,
    ...
    # create loss
    # all_gather需要修改batch_size, * world_size  # world_size = dist.get_world_size()
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            # text_generation_optimizer.load_state_dict(checkpoint['text_generation_optimizer'])
            # fusion_optimizer.load_state_dict(checkpoint['fusion_optimizer'])
            # video_fusion_optimizer.load_state_dict((checkpoint['video_fusion_optimizer']))
    ...
    if args.evaluate_query2goods_query:
        test_loader = get_loaders(args)

        evaluate_query2goods_query(test_loader, doc_text_model, args)
        exit(0)

    if args.evaluate_query2goods_goods:
        test_loader = get_loaders(args)
        evaluate_query2goods_goods(test_loader, doc_text_model, doc_image_model, doc_fusion_model, args)
        exit(0)
...
def get_loaders(args, epoch=0):
    ...
    if args.evaluate:
        test_dataset = CrossDomainEmbDatasetV1(args.test_file, args.clip_length,
                                               args.goods_img_root, args.goods_text_root,
                                               args.photo_img_root, args.photo_text_root,
                                               args.live_img_root, args.live_text_root,
                                               shuffle=False, transform=val_transform, return_img_id=True,
                                               is_train=False)
        test_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset)
        test_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=args.batch_size, shuffle=(test_sampler is None),
            num_workers=args.workers, pin_memory=True, sampler=test_sampler, drop_last=False, collate_fn=CollateFn())
        return test_loader, test_sampler

    if args.evaluate_live2goods:
        pass

    if args.evaluate_query2goods_query:
        test_dataset = Query2GoodsDataset_query(args.test_file)
        test_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset)
        test_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=args.batch_size, shuffle=(test_sampler is None), num_workers=args.workers,
            pin_memory=True, sampler=test_sampler, drop_last=False, collate_fn=CollateFnQuery()
        )
        return test_loader

    if args.evaluate_query2goods_goods:
        test_dataset = Query2GoodsDataset_goods(args.test_file, args.goods_img_root, args.goods_text_root,
                                                transform=val_transform)
        test_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset)
        test_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=args.batch_size, shuffle=(test_sampler is None), num_workers=args.workers,
            pin_memory=True, sampler=test_sampler, drop_last=False, collate_fn=CollateFnGoods()
        )
        return test_loader
